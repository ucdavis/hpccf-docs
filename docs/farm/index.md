...
- The `low` (low priority/requeue) partition, which has ~100 nodes with a combined ~15,000 CPUs, ~64 TB of RAM, and ~50 GPUs. Jobs in `low` are limited to a maximum runtime of 7 days and will be killed and requeued if the resources are needed for jobs in the `high` partition.

In addition to this, each new user is allocated a 20 GB home directory. If you want to use the CA&ES free tier, select "CA&ES free tier" from the list of sponsors [here](https://hippo.ucdavis.edu/Farm/myaccount).
